...
from ptagnn import data
from ptagnn.modules.utils import extract_invariant,get_edge_vectors_and_lengths
from ptagnn.tools import torch_geometric, torch_tools, utils
...
class MACECalculator(Calculator):
    ...
    def get_effectivemji(self, atoms=None,num_layers=-1):
        """Extracts the descriptors from MACE model.
        :param atoms: ase.Atoms object
        :param invariants_only: bool, if True only the invariant descriptors are returned
        :param num_layers: int, number of layers to extract descriptors from, if -1 all layers are used
        ...
        effective_mjis = [model(batch.to_dict(),show_radial_distribution=True)["effective_mji"] for model in self.models]
        effective_mjis = [effective_mji.detach().cpu().numpy() for effective_mji in effective_mjis]
        if self.num_models == 1:
            return effective_mjis[0],batch.edge_index[0],batch.edge_index[1],lengths
        return effective_mjis,batch.edge_index[0],batch.edge_index[1],lengths

    def get_nodeenergy(self, atoms=None,num_layers=-1):
        """Extracts the descriptors from MACE model.
        :param atoms: ase.Atoms object
        :param invariants_only: bool, if True only the invariant descriptors are returned
        :param num_layers: int, number of layers to extract descriptors from, if -1 all layers are used
        ...
        descriptors = [model(batch.to_dict())["node_energy"] for model in self.models]
        descriptors = [descriptor.detach().cpu().numpy() for descriptor in descriptors]
    ...
    def get_node_e0(self, atoms=None, num_layers=-1):
        """Extracts the descriptors from MACE model.
        :param atoms: ase.Atoms object
        :param invariants_only: bool, if True only the invariant descriptors are returned
        :param num_layers: int, number of layers to extract descriptors from, if -1 all layers are used
        ...
        for model in self.models:
            output = model(batch.to_dict())
        descriptors = [model(batch.to_dict(),show_radial_distribution=True)["node_e0"] for model in self.models]
        descriptors = [descriptor.detach().cpu().numpy() for descriptor in descriptors]
    ...
    def get_interaction_node_energy(self, atoms=None, num_layers=-1):
        """Extracts the descriptors from MACE model.
        :param atoms: ase.Atoms object
        :param invariants_only: bool, if True only the invariant descriptors are returned
        :param num_layers: int, number of layers to extract descriptors from, if -1 all layers are used
        ...
        for model in self.models:
            output = model(batch.to_dict())
        descriptors = [model(batch.to_dict())["interaction_node_energy"] for model in self.models]
        descriptors = [descriptor.detach().cpu().numpy() for descriptor in descriptors]
