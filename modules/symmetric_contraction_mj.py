...
from ptagnn.tools.cg import U_matrix_real
...
class SymmetricContraction(CodeGenMixin, torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor,mji,receiver):
        outs = []
        outs_basis = {}
        if mji is not None:
           mjis=[]
        ...
        for contraction in self.contractions:
            tmp=contraction(x, y, mji, receiver)
            outs.append(tmp[0])
            outs_basis.update(tmp[1])
            if mji is not None:
                mjis.append(tmp[2])
        return torch.cat(outs, dim=-1),outs_basis,mjis
    ...
    def __init__(
        ...
        if self.coupling_irreps.dim == 1:
            for nu in range(1, correlation + 1):
                size = [1 for i in range(nu + 1)]
                U_matrix = torch.ones(size=size)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor,mji,receiver):
        ...
        for i, (weight, contract_weights, contract_features) in enumerate(
            ...
            if i == len(self.weights)-1:
                c_tensor_last = c_tensor
            out = contract_features(c_tensor, x)
        resize_shape = torch.prod(torch.tensor(out.shape[1:]))

        ##### correlation #####
        correlation_basis = {}
        correlation_basis[self.correlation]=(
            self.graph_opt_main(
            self.U_tensors(self.correlation),
            self.weights_max,
            x,
        ...
        for i, (weight, contract_weights, contract_features) in enumerate(
            ...
            correlation_basis[self.correlation - i - 1] = \
                (contract_weights(
                self.U_tensors(self.correlation - i - 1),
                weight,
                y,
            ))
            for key in correlation_basis.keys():
                correlation_basis[key]=contract_features(correlation_basis[key], x)

        if mji is not None:
            mji = self.contractions_features[-1](c_tensor_last[receiver,:,:],mji)
        return out.view(out.shape[0], resize_shape),correlation_basis,mji
