...
class EquivariantProductBasisBlock(torch.nn.Module):
    ...
    def forward(
        ...
        node_feats, node_feats_basis, mji2 = self.symmetric_contractions(node_feats, node_attrs,mji, receiver)
        if mji is None and receiver is None:
            if self.use_sc and sc is not None:
                return self.linear(node_feats) + sc, node_feats_basis
            else:
                return self.linear(node_feats), node_feats_basis
        else:
            if self.use_sc and sc is not None:
                return self.linear(node_feats) + sc, node_feats_basis, mji2
            else:
                return self.linear(node_feats), node_feats_basis, mji2
...
class Residual_InteractionBlock(InteractionBlock):
    ...
    def forward(
        ...
        if self.show_radial_distribution:
            mji2 = self.linear(mji) / self.avg_num_neighbors
            #mji2 = self.skip_tp(mji2, node_attrs[receiver])
            mji2 = self.reshape(mji2)
            return (
